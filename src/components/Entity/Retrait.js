...
function Retrait() {
 ...
 // const [b, setB] = useState(utilisateur.id);
  ...
  async function Listeretrait() {
    let result = await fetch("http://localhost:8000/api/listeretraits");
    result = await result.json();
    setData4(result);
  ...
  async function ajout_entite() {
    ...
    formData.append("montant_retrait", adresseentite);
    formData.append("date_retrait", date);
    formData.append("user", user);
    
   
    let result = await fetch("http://localhost:8000/api/ajoutretraits", {
      method: "Post",
...
async function ajoutpersonne(data){
  ...
  async function Suprimerentite(id) {
    let result = await fetch(
      "http://localhost:8000/api/supprimerretraits/" + id,
      {
        method: "DELETE",
      }
    );
  ...
  async function modifierentite(id) {
    ...
    formData.append("montant_retrait", adressemodifier);
    formData.append("date_retrait", datemodif);
    formData.append("user", user);

    let result = await fetch(
      "http://localhost:8000/api/modificationretraits/" + id + "?_method=PUT",
      {
        method: "Post",
  ...
  async function Passeentite(id) {
    handleShow();
    let result = await fetch("http://localhost:8000/api/obtclientretr/" + id);
    result = await result.json();
    setData1(result);
    setEntitemodifier(result.numcompte);
    setAdressemodifier(result.montant_retrait);
    setDatemodif(result.date_retrait);
      ...
      return (
        <>
          ...
          <Modal.Footer>
            ...
            <Button
              ...
              onClick={() => modifierentite(data1.numcheque)}
            >
              Enregistrer la modification
            </Button>
            <Button
              class="btn  btn-outline-danger"
              onClick={() => Suprimerentite(data1.numcheque)}
            >
              Supprimer
            </Button>
          </Modal.Footer>
        ...
        <Modal.Title class="text-center bg-dark text-white"></Modal.Title>
          ...
          <Modal.Footer>
            ...
            <Button
              class="btn  btn-outline-danger"
              onClick={() => Suprimerentite(data3.numcheque)}
            >
              Supprimer
            </Button>
          </Modal.Footer>
