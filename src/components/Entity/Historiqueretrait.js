...
function Historiqueretrait() {
    ...
    React.useEffect(() => {
      ...
      async function ListeEntite() {
        let result = await fetch("http://localhost:8000/api/auditretraits");
...
function Etat2(etat, a, b, c, d, e, f, g, h, i, j, k) {
  ...
  if (etat=='insertion') {
    return ' Utilisateur ID : ' + a +' , Numéro  de chèque : ' + b + '\n \n,  montant retrait : ' + c + ' solde précedement : ' + i +' Reste = '+ j  + ' , date : ' + convertirdate(k)
   
  } 

  if (etat=='Modification') {
    
    return ' Utilisateur ID : ' + a +' ,  Numéro  de chèque : ' + b + ' , montant retrait : ' + c + ' solde précedement : ' + i +' Reste = '+ j  + ' , date : ' + convertirdate(k)
  } 
        ...
        <>
          ...
          <Sidebar/>
            <div className="main-panel" ref={mainPanel}>
              ...
              <h2 style={{textAlign:'center'}}>Historique des retraits</h2>
              <Container>
              <TextField
                id="outlined-basic"
                label="Rechercher"
...
export default Historiqueretrait